...
仓库提供三个不修改 hydromodel 核心代码的辅助工具：

| 工具 | 命令 | 说明 |
|------|------|------|
| 统一入口 | `python scripts/songliao_workflow.py doctor` | 从仓库根目录检查环境、打印或运行标准流程 |
| 实验对比 | `python scripts/songliao_workflow.py compare` | 扫描已有实验并按 NSE 生成 CSV/Markdown 排名 |
| 提交自检 | `python scripts/check_songliao_submission.py` | 核验配置、指标、参数和图件，不重跑率定 |

详细参数见[统一入口](docs/songliao-workflow-launcher-guide.md)、
[实验对比](docs/songliao-experiment-comparison-guide.md)和
[提交自检](docs/songliao-submission-audit-guide.md)。
