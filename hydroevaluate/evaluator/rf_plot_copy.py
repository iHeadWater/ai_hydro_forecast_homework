...
def plot_predobs_with_tp_hydro(
    ...
    obs = obs_ds.sel(basin=target_basin_id)[pred_colunm].sel(
        time=slice(time_start, time_end)
    )
    pred = pred_ds.sel(basin=target_basin_id)[pred_colunm].sel(
        time=slice(time_start, time_end)
    )
    # regional预测流量
    if regional_nc is not None:
        regional_ds = (
            xr.open_dataset(regional_nc)
            if isinstance(regional_nc, str)
            else regional_nc
        )
        regional_pred = regional_ds.sel(basin=target_basin_id)[pred_colunm].sel(
            time=slice(time_start, time_end)
        )
    else:
        regional_pred = None
    ...
    # 4. 单位换算
    basin_area = (
        station_dict[target_basin_id]["area"]
        if target_basin_id in station_dict
        else None
    )
    ...
    def is_valid_series(series):
        return (
            hasattr(series, "__len__") and len(series) == len(time) and len(series) > 0
        )
    ...
    try:
        ...
        # 如果regional指标存在，追加显示
        if (
            regional_peak_bias is not None
            or regional_volume_bias is not None
            or regional_peak_time_bias is not None
        ):
            info_str += (
                "\nregional:\n"
...
def plot_flood_event_with_hydro(
    ...
    csv_path = os.path.join(
        data_path, "timeseries", time_unit, f"{target_basin_id}.csv"
    )
    ...
    try:
        dataset = SelfMadeHydroDataset(
            data_path=data_path, time_unit=[time_unit], offset_to_utc=True
        )
