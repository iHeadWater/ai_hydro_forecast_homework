...
def plot_precip_flow(
    ...
    try:
        ...
        if target_basin_id in ds[basin_columns].values:
            ...
            flow_obs = ds.sel({basin_columns: target_basin_id})["streamflow"]
            ...
            # 读取流量预测和实测数据
            flow_obs = ds.sel({basin_columns: target_basin_id}).sel(
                time=slice(flow_time_start, flow_time_end)
            )["streamflow"]
            flow_pred = (
            ...
            ax2.set_ylabel("流量（m^3/s）", color="red", fontproperties=font_prop)
            ax2.tick_params(axis="y", labelcolor="red")

            # 设置标题和图例
            plt.title(
                f"{target_basin_id}水文站 降雨与流量时序图",
                fontproperties=font_prop,
            )

            plt.legend(loc="upper left")

            plt.savefig(f"{output_folder}/{target_basin_id}_streamflow.png")
...
def plt_by_nc(pred_nc, basin_ids, basin_info_csv, time_unit, project_name, time_range):
    output_folder = os.path.join(RESULT_DIR, project_name)
    basin_colunms = "basin"
    precip_var = "total_precipitation_hourly"
    flow_var_pred = pred_nc
