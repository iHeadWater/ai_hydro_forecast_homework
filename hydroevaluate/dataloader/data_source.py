...
# TODO: We do not need all functions in SelfMadeHydroDataset, and we need to reorganize some functions
class SelfMadeHydroDatasetForEval:
    ...
    def read_attributes(
        self, object_ids=None, constant_vars=None, **kwargs
    ) -> np.array:
        ...
        if object_ids is None:
            if constant_vars is None:
                return attrs
            object_ids = attrs["basin_id"].values
        if constant_vars is None:
            constant_vars = attrs.columns.values
        x = np.full([len(object_ids), len(constant_vars)], np.nan)
        for k in range(len(object_ids)):
            ind = attrs["basin_id"] == object_ids[k]
            for j in range(len(constant_vars)):
                x[k, j] = attrs[constant_vars[j]][ind].values
        return x
